...
const useWallpaper = (
  ...
  const resetWallpaper = useCallback(
    (keepCanvas?: boolean): void => {
      ...
      if (!["IMAGE", "SLIDESHOW"].includes(wallpaperName)) {
    ...
    async (keepCanvas?: boolean) => {
      ...
      if (
        ...
        const fallbackWallpaper = (): void =>
          setWallpaper(wallpaperName === "VANTA" ? "IMAGE" : "VANTA");
  ...
  const loadFileWallpaper = useCallback(async () => {
    ...
    const isImage = wallpaperName === "IMAGE";
    const isSlideshow = wallpaperName === "SLIDESHOW";

    if (isImage) {
      const DEFAULT_BACKGROUND_IMAGE = "background.jpg";
      const backgroundImageFilePath = `${PICTURES_FOLDER}/${DEFAULT_BACKGROUND_IMAGE}`;
      wallpaperUrl = backgroundImageFilePath;
